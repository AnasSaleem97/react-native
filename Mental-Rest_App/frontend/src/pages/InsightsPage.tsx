...
const InsightsPage = () => {
  ...
  const fetchInsightsData = async () => {
    try {
      ...
      // Fetch AI insights
      const insightsResponse = await axios.get(`${API_BASE_URL}/insights`, {
        headers: { Authorization: `Bearer ${token}` }
      });
            ...
            <div className="bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 rounded-3xl p-8 text-white shadow-xl">
              <div className="flex items-center mb-8">
                ...
                <div>
                  ...
                  <p className="text-indigo-100">Personalized analysis of your wellness journey</p>
                </div>
              </div>
              ...
              {aiInsights.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-indigo-100">No insights available yet</p>
                  <p className="text-indigo-200 text-sm">Keep journaling to unlock AI insights</p>
                </div>
              ) : (
                <div className="space-y-6">
                    ...
                    <div key={index} className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20 hover:bg-white/20 transition-all duration-300">
                      <div className="flex items-start space-x-4">
                        ...
                        <div>
                          ...
                          <p className="text-indigo-100 leading-relaxed">{insight.content}</p>
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            <div className="bg-white/70 backdrop-blur-sm rounded-3xl p-8 shadow-xl border border-white/20">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-gray-800">🤖 AI-Powered Wellness Tips</h3>
                    <p className="text-gray-600">Personalized recommendations based on your mood patterns</p>
                  </div>
                </div>
                <div className="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-3">
                  <select
                    value={currentTipCategory}
                    onChange={(e) => setCurrentTipCategory(e.target.value)}
                    className="px-4 py-2 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-purple-500/20 focus:border-purple-500 transition-all duration-300 bg-white/50 backdrop-blur-sm"
                  >
                  ...
                  <button
                    onClick={() => generateAIWellnessTips(currentTipCategory, true)}
                    disabled={isGeneratingTips}
                    className="px-6 py-3 bg-gradient-to-r from-purple-500 to-pink-600 text-white font-semibold rounded-2xl shadow-lg hover:shadow-xl transform hover:-translate-y-1 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
                  >
                    {isGeneratingTips ? (
                      <div className="flex items-center">
                        <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin mr-2"></div>
              ...
              {isGeneratingTips ? (
                <div className="text-center py-16">
                  ...
                  <h4 className="text-xl font-semibold text-gray-800 mb-3">🤖 AI is Generating Fresh Tips...</h4>
                  <p className="text-gray-600 mb-4">Our AI is analyzing your mood patterns and creating personalized recommendations</p>
                ...
                <div className="space-y-6">
                  {/* Error Message */}
                  {aiWellnessTips.error && (
                    <div className="bg-gradient-to-r from-yellow-50 to-orange-50 rounded-2xl p-4 border border-yellow-200">
                      <div className="flex items-center">
                        <span className="text-2xl mr-3">⚠️</span>
                        <p className="text-gray-700 text-sm">{aiWellnessTips.error}</p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl p-6 border border-purple-200">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center">
                        ...
                        <h4 className="font-semibold text-gray-800">Focus Area</h4>
                      </div>
                      <div className="flex items-center space-x-2">
                        {aiWellnessTips.isAIGenerated ? (
                          <span className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded-full">🤖 AI Generated</span>
                        ) : (
                          <span className="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">📚 Curated</span>
                        )}
                      </div>
                    </div>
                    <p className="text-gray-700 font-medium capitalize">{aiWellnessTips.focus}</p>
                  </div>

                  {/* AI Tips */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {aiWellnessTips.tips.map((tip: string, index: number) => (
                      <div key={index} className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-6 border border-blue-200 hover:shadow-lg transition-all duration-300">
                  ...
                  <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-2xl p-6 border border-emerald-200">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center">
                        <span className="text-2xl mr-3">💪</span>
                        <h4 className="font-semibold text-gray-800">AI Encouragement</h4>
                      </div>
                      <div className="text-xs text-gray-500">
                        Generated: {new Date(aiWellnessTips.generatedAt).toLocaleTimeString()}
                      </div>
                    </div>
                    <p className="text-gray-700 italic mb-4">"{aiWellnessTips.encouragement}"</p>
                    
                    {/* More Tips Button */}
                    <div className="flex flex-col sm:flex-row items-center justify-between space-y-3 sm:space-y-0 sm:space-x-4">
                      <div className="flex items-center space-x-2">
                        <span className="text-sm text-gray-600">Want different tips?</span>
                        <select
                          value={currentTipCategory}
                          onChange={(e) => setCurrentTipCategory(e.target.value)}
                          className="px-3 py-1 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500"
                        >
                          {tipCategories.map(category => (
                            <option key={category.value} value={category.value}>
                              {category.emoji} {category.label}
                      ...
                      <button
                        onClick={() => generateAIWellnessTips(currentTipCategory, true)}
                        disabled={isGeneratingTips}
                        className="px-4 py-2 bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-medium rounded-xl shadow-md hover:shadow-lg transform hover:-translate-y-0.5 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
                      >
                        {isGeneratingTips ? (
                          <div className="flex items-center">
                ...
                <div className="text-center py-12">
                  ...
                  <h4 className="text-xl font-semibold text-gray-800 mb-3">Ready for AI-Powered Tips?</h4>
                  <p className="text-gray-600 mb-6">Click "Get AI Tips" to receive personalized wellness recommendations based on your journal entries and mood patterns.</p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl mx-auto">
                    <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-4 border border-blue-200">
                      ...
                      <p className="text-gray-600 text-sm">AI analyzes your mood patterns and journal content</p>
                    </div>
                    <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl p-4 border border-green-200">
                      <div className="flex items-center mb-2">
                        <span className="text-xl mr-2">🎯</span>
                        <h5 className="font-semibold text-gray-800">Personalized</h5>
                      </div>
                      <p className="text-gray-600 text-sm">Tips tailored specifically to your mental wellness journey</p>
                    </div>
                  </div>
                </div>
              )}
